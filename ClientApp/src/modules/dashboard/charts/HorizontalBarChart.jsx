...
const HorizontalBarChart = ({ config = {}, width = 500, height = 350, globalFilters = {} }) => {
    ...
    <div className="w-full h-full overflow-hidden flex flex-col" style={{ background: 'hsl(var(--card))' }}>
      <div className="flex-1 min-h-0">
        <ResponsiveContainer width="100%" height="100%">
          <RechartsBarChart data={data} layout="vertical" margin={{ ...CHART_MARGINS.large, left: 120 }}>
            <CartesianGrid strokeDasharray="6 6" opacity={0.25} />
            <XAxis type="number" tick={AXIS_PROPS.tick} tickFormatter={formatNumber} axisLine={false} tickLine={false} />
            <YAxis dataKey="name" type="category" width={110} tick={AXIS_PROPS.tick} axisLine={false} tickLine={false} />
            <Tooltip content={<CustomTooltip />} cursor={false} />
            {dataKeys.length > 1 && <Legend verticalAlign="top" height={26} />}
            
            {dataKeys.map((key, i) => (
              <Bar key={key} dataKey={key} fill={CHART_COLORS[i % CHART_COLORS.length]} name={key} isAnimationActive animationDuration={1200} />
            ))}
          </RechartsBarChart>
        </ResponsiveContainer>
      </div>
