  ...
  return (
    <div className={`min-h-screen flex items-center justify-center ${isDark ? 'bg-slate-950' : 'bg-gradient-to-br from-blue-50 via-indigo-50 to-blue-50'}`}>
      <div className={`rounded-2xl shadow-xl w-full max-w-md p-8 text-center ${isDark ? 'bg-slate-900 border border-blue-500/20' : 'bg-white border border-blue-200'}`}>
        <div className={`rounded-full w-10 h-10 flex items-center justify-center text-lg font-bold mb-2 mx-auto text-white bg-gradient-to-br from-blue-600 to-indigo-600`}>S</div>
        <div className={`font-semibold text-lg mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>SureOps</div>
        <div className={`text-sm ${isDark ? 'text-gray-400' : 'text-slate-600'}`}>{message}</div>
      </div>
    </div>
  );
};
