...
const API = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white dark:bg-slate-900 rounded-lg shadow p-8 relative border border-gray-200 dark:border-slate-700">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-2xl font-bold text-primary">API Keys</h2>
        <button
          className="px-4 py-2 rounded bg-blue-600 dark:bg-blue-700 text-white font-semibold hover:bg-blue-700 dark:hover:bg-blue-800 transition-colors"
      ...
      <table className="w-full border border-gray-200 dark:border-slate-700 rounded">
        <thead>
          <tr className="bg-gray-100 dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700">
            ...
            <th className="p-2 text-left text-primary">Created At</th>
            <th className="p-2 text-left text-primary">Last Used</th>
            <th className="p-2 text-left text-primary">Permissions</th>
            <th className="p-2 text-left text-primary">Expires At</th>
            <th className="p-2 text-left text-primary">Status</th>
          </tr>
        </thead>
        <tbody>
          {paginatedApiKeys.map(key => (
            <tr key={key.key_id} className="border-t border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-800/50">
              <td className="p-2 text-secondary">{key.name}</td>
              <td className="p-2 text-secondary">{key.created_at ? new Date(key.created_at).toLocaleString() : '-'}</td>
              <td className="p-2 text-secondary">{key.last_used ? new Date(key.last_used).toLocaleString() : '-'}</td>
              <td className="p-2 text-secondary">{key.permissions.join(', ')}</td>
              <td className="p-2 text-secondary">{key.expires_at ? new Date(key.expires_at).toLocaleString() : '-'}</td>
              <td className="p-2 text-secondary">{key.status}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div className="flex items-center justify-between mt-4">
        <div className="flex items-center gap-2">
          <label htmlFor="rows-per-page" className="text-sm text-secondary">Rows per page</label>
          <select
            id="rows-per-page"
